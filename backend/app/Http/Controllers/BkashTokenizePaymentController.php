...
class BkashTokenizePaymentController extends Controller
{
    ...
    public function callBack(Request $request)
    {
        ...
        if (!$orderId) {
            return BkashPaymentTokenize::failure('Order ID missing');
        }

        $order = \App\Models\Order::find($orderId);
        if (!$order) {
            return BkashPaymentTokenize::failure('Order not found');
        }
        ...
        if ($request->status == 'success'){
            ...
            if (isset($response['statusCode']) && $response['statusCode'] == "0000" && $response['transactionStatus'] == "Completed") {
                ...
                return redirect()->away(env('FRONTEND_URL', 'http://localhost:3000') . '/order-success?txn=' . $response['trxID']);
            }
            // Payment failed
            ...
            \App\Models\Transaction::create([
                'order_id' => $order->id,
                'gateway' => 'bkash',
                'gateway_txn_id' => $request->paymentID,
                'amount_cents' => $order->total_cents,
                'status' => 'failed',
                'raw_response' => $response,
            ]);
            ...
            return redirect()->away(env('FRONTEND_URL', 'http://localhost:3000') . '/checkout?error=payment_cancelled');
        }else{
            $order->update(['status' => 'failed']);
            return redirect()->away(env('FRONTEND_URL', 'http://localhost:3000') . '/checkout?error=payment_failed');
        }
